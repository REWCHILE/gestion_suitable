    ...
    * {
    ...
    .login-card:hover {
      box-shadow: 0 30px 70px -15px rgba(15, 23, 42, 0.18),
                  ...
                  0 0 0 2px rgba(30, 136, 136, 0.14);
    }
    .login-header {
      text-align: center;
      margin-bottom: 28px;
...
<body>
  ...
  <!-- CANVAS ANIMADO REACTIVO AL MOUSE -->
  ...
  <div class="login-card">
    ...
    <form method="POST" action="{{ route('login') }}">
      ...
      <div class="form-group">
        ...
        <input type="email" id="email" name="email" class="form-control" placeholder="admin@suitable.cl" value="{{ old('email') }}" required autofocus>
      </div>
  ...
  <script>
    (function() {
      ...
      // PALETA DE COLORES CORPORATIVOS SUITABLE
      const colors = [
        'rgba(30, 136, 136, 0.75)',   // Teal Suitable #1E8888
        'rgba(13, 148, 136, 0.70)',   // Teal Profundo
        'rgba(20, 184, 166, 0.80)',   // Cyan Clínico
        'rgba(45, 212, 191, 0.85)',   // Menta Brillante
        'rgba(56, 189, 248, 0.65)'    // Celeste Suave
      ];

      // CANTIDAD Y CREACIÓN DE PARTÍCULAS
      ...
      class Particle {
        ...
        update() {
          // Movimiento armónico continuo
          this.angle += this.angleSpeed;
          this.x += this.vx + Math.cos(this.angle) * 0.25;
          this.y += this.vy + Math.sin(this.angle) * 0.25;

          // Suavizado / fricción de regreso a velocidad base
          this.vx += (this.baseVx - this.vx) * 0.04;
          this.vy += (this.baseVy - this.vy) * 0.04;

          // REACCIÓN AL MOUSE (REPULSIÓN ELÁSTICA DINÁMICA)
          ...
          // REACCIÓN A LAS ONDAS DE CHOQUE (CLIC)
          ...
          // Rebotar o envolver bordes suavemente
      ...
      function animate() {
        ...
        // DIBUJAR ONDAS DE CHOQUE EXPANSIVAS
        ...
        // LÍNEAS DE CONSTELACIÓN ENTRE PARTÍCULAS
        const maxDist = 125;
        for (let i = 0; i < particles.length; i++) {
          ...
          // LÍNEAS LUMINOSAS CONECTADAS DIRECTAMENTE AL MOUSE
          if (mouse.active && mouse.x !== null) {
