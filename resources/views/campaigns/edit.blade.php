    ...
    <div class="form-group" style="margin-bottom: 22px; background: #F8FAFC; border: 1.5px solid #E2E8F0; border-radius: 12px; padding: 18px;">
      ...
      <div id="presets_carousel_track" style="display: flex; gap: 10px; overflow-x: auto; scroll-behavior: smooth; padding-bottom: 8px; scrollbar-width: thin;">
        @foreach($allPresets as $p)
          @php $isActive = ($selectedPresetId ?? 'clasica') === $p['id']; @endphp
          <div class="preset-card {{ $isActive ? 'active' : '' }}" 
               id="preset_card_{{ $p['id'] }}" 
               onclick="selectPreset('{{ $p['id'] }}')"
               style="flex: 0 0 155px; min-width: 155px; padding: 12px 10px; border: 2px solid {{ $isActive ? '#1E8888' : '#E2E8F0' }}; border-radius: 8px; background: white; cursor: pointer; text-align: center; position: relative; transition: all 0.2s ease;">
            ...
            <span class="preset-badge" style="display: {{ $isActive ? 'block' : 'none' }}; position: absolute; top: 4px; right: 4px; background: #1E8888; color: white; font-size: 8px; font-weight: 800; padding: 1px 4px; border-radius: 4px;">ACTIVO</span>
          </div>
        @endforeach
      </div>
