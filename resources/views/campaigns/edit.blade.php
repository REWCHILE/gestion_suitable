...
@section('title', 'Editar Campaña #' . $campaign->id . ' (Canvas StampReady con Asistentes IA Contextuales) | Suitable')
...
<div class="sr-topbar">
  <div style="display: flex; align-items: center; gap: 12px;">
    ...
    <input type="text" id="sr_campaign_name" value="{{ $campaign->name }}" style="border: 1px solid #CBD5E1; border-radius: 6px; padding: 6px 10px; font-size: 13px; font-weight: 800; color: #0F172A; width: 280px;" title="Nombre interno de la campaña">
    <span class="badge" style="background: #E6F4F4; color: #115E59; font-size: 11px; font-weight: 800; padding: 5px 10px; border-radius: 6px;">
      ID #{{ $campaign->id }}
    </span>
  </div>
    ...
    <div id="panel_modules" style="display: none;">
      ...
      <div style="margin-top: 18px; padding-top: 14px; border-top: 1px solid #E2E8F0;">
        ...
        <div class="form-group" style="margin-bottom: 8px;">
          ...
          <input type="text" id="sr_subject_input" class="form-control" value="{{ $campaign->subject }}" style="font-size: 11.5px;" oninput="syncSubjectToCanvas()">
        </div>
        <div class="form-group" style="margin-bottom: 0;">
          <label style="font-size: 11px; font-weight: 700;">Preheader:</label>
          <input type="text" id="sr_preheader_input" class="form-control" value="{{ $campaign->preheader }}" style="font-size: 11.5px;" oninput="syncSubjectToCanvas()">
        </div>
      </div>
    </div>
    ...
    <div id="panel_crm" style="display: none;">
      ...
      <div class="form-group">
        ...
        <select id="sr_group_id" class="form-control" style="font-size: 12px;">
          ...
          @foreach($groups as $g)
            <option value="{{ $g->id }}" {{ (int)$campaign->group_id === (int)$g->id ? 'selected' : '' }}>{{ $g->name }} ({{ $g->clients_count }})</option>
          @endforeach
        </select>
      </div>
    </div>
    ...
    <div class="sr-email-canvas" id="sr_email_canvas">
      ...
      <div class="sr-module-block" id="mod_hero" data-sr-type="module" data-sr-label="Portada Principal (Hero)">
        ...
        <div class="sr-detect-image" onclick="selectImageElement('hero', 'Fotografía Portada Hero')" data-sr-type="image" data-sr-label="Fotografía Portada Hero" title="Haz clic para generar o cambiar esta fotografía con IA">
          ...
          <img id="el_hero_img" src="{{ asset('images/' . ($campaign->hero_image ?: 'hero-grupo-clinico.jpg')) }}" alt="Hero Banner" style="width: 100%; height: auto; display: block; object-fit: cover; max-height: 280px;">
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 12px; text-align: center;">
          ...
          <div style="background: #F8FAFC; border: 1px solid #E2E8F0; border-radius: 8px; padding: 12px; display: flex; flex-direction: column; justify-content: space-between;">
            <div>
              ...
              <strong class="sr-detect-text" id="el_p1_title" data-sr-type="text" data-sr-label="Título Pilar 1" style="font-size: 12px; color: #0F172A; display: block;">{{ $campaign->pilar1_title ?: 'Scrub Flex Ergonómico' }}</strong>
              ...
              <p class="sr-detect-text" id="el_p1_desc" data-sr-type="text" data-sr-label="Descripción Pilar 1" style="font-size: 10.5px; color: #64748B; line-height: 1.4; margin: 0 0 10px 0;">{{ $campaign->pilar1_desc ?: 'Confección directa sin intermediarios con 6 meses de garantía.' }}</p>
            </div>
            <div>
          ...
          <div style="background: #F8FAFC; border: 1px solid #E2E8F0; border-radius: 8px; padding: 12px; display: flex; flex-direction: column; justify-content: space-between;">
            <div>
              ...
              <strong class="sr-detect-text" id="el_p2_title" data-sr-type="text" data-sr-label="Título Pilar 2" style="font-size: 12px; color: #0F172A; display: block;">{{ $campaign->pilar2_title ?: 'Delantal Antifluido' }}</strong>
              ...
              <p class="sr-detect-text" id="el_p2_desc" data-sr-type="text" data-sr-label="Descripción Pilar 2" style="font-size: 10.5px; color: #64748B; line-height: 1.4; margin: 0 0 10px 0;">{{ $campaign->pilar2_desc ?: 'Barrera biosegura contra salpicaduras y fluidos corporales.' }}</p>
            </div>
            <div>
          ...
          <div style="background: #F8FAFC; border: 1px solid #E2E8F0; border-radius: 8px; padding: 12px; display: flex; flex-direction: column; justify-content: space-between;">
            <div>
              ...
              <strong class="sr-detect-text" id="el_p3_title" data-sr-type="text" data-sr-label="Título Pilar 3" style="font-size: 12px; color: #0F172A; display: block;">{{ $campaign->pilar3_title ?: 'Tallaje en Terreno' }}</strong>
              ...
              <p class="sr-detect-text" id="el_p3_desc" data-sr-type="text" data-sr-label="Descripción Pilar 3" style="font-size: 10.5px; color: #64748B; line-height: 1.4; margin: 0 0 10px 0;">{{ $campaign->pilar3_desc ?: 'Percheros móviles en su clínica para probar tallas en vivo.' }}</p>
            </div>
            <div>
...
<script>
  ...
  // --- 8. ASISTENTE DE IMAGEN CONTEXTUAL INTELIGENTE ---
  ...
  async function saveStampReadyCampaign() {
    ...
    const payload = {
      ...
      preset_template: '{{ $campaign->preset_template ?: "clasica" }}',
